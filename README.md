...
| 🗺️ | **Silence Map** | Leaflet map of all heard stations with real APRS symbols; detects regions where stations fall silent together (per-station beacon-cadence baseline, Maidenhead cell clustering, igate-failure discrimination) and paints affected cells. **An alert means the silence is news, not merely that the threshold was met.** Three things demote a cell out of the alert list — and off the notifications and the AI spend — while leaving it on the map: a *chronic* cell, whose silent stations are all ones it habitually misses; a cell whose silent callsigns belong to too few separate operators, since three SSIDs of one base callsign are three radios in one shack on one power strip and cannot fail independently; the proportion a cell reports counts operators at **both** ends for the same reason — a cell of two operators with one silent is not half a region gone quiet, however many SSIDs that operator runs, and the callsign figure travels beside it as `ratio_callsigns` because fourteen days of stored snapshots were computed with it; and a cell where **no igate ever heard any of them** — every station either gated itself or dialled an APRS-IS backbone server directly, which makes their silence an internet or app dropout rather than anything about a region. That last one is not rare: on the worldwide feed it accounted for 23 of 36 cells, all of them previously reported as regional outages. What keeps a cell alerting is a station that is *unusual there* — one present in under 35 % of that cell's own past alerts — and the popup names it, so the reason is a callsign rather than a colour: *"New here: YM1ABC-7 — normally not among this cell's missing stations."* The raw threshold result is still reported as `threshold_met`, so nothing is hidden. Note that the stored history keeps only the cells that alerted — it replays what was announced, and cannot say what the thresholds turned away. Cell colour separates the cases: red for a regional silence, yellow when a shared igate was **seen** to go quiet, orange when one gate carries every silent station but cannot be seen at all, purple when nothing local ever observed them, grey for measured but not announced. Popups also flag silent stations whose own beacon cannot be trusted about where they are — a US callsign transmitting an eastern longitude is a well-known hotspot misconfiguration, and a cell built out of those is not describing the region it is drawn on. If the APRS-IS feed itself stops, nothing is judged at all: the page says so and holds the last confirmed reading rather than reporting the world as silent. Timeline slider replays the last 14 days of snapshots; new alerts get an AI assessment (via AI Gateway) shown in cell popups and are sent to the Monitor notify channel (Telegram/email). The agent's own Fixed Beacon is shown on the map but excluded from silence detection (it is self-generated and would otherwise be a phantom "still active" vote). Detection can be scoped to your own region with `monitor.silence_grids` (Maidenhead fields, e.g. `["KM","KN","LM","LN"]` for Turkey) — a prefix station filter such as `p/TA` also matches callsigns abroad, and without this their outages raise alerts too. For worldwide monitoring, `monitor.silence_digest_mins` batches alerts into one combined notification per interval. Event advisories are never counted as silence sensors, in either form they arrive: APRS **Objects**, which expire by design, and the 557 weather-service products that arrive as ordinary **stations** — one per office per product, transmitting while a warning is in force and silent for the days between. Silence detection rests entirely on cadence and those have none to fall silent against, so before they were excluded a cell of expired storm warnings reported fair weather as a regional outage |
| 📡 | **RF Propagation Tracking** | Every qAR/qAO-gated packet is one realised RF link whose length is known exactly (sender's in-packet position to the igate's position). Per-gate baselines separate "this mountain-top gate always hears far" from "the band just opened"; abnormally long links draw as dashed great-circle lines on the map, colour-tiered by distance. **Those baselines survive a restart** — before they did, no gate ever reached the 20 samples its own threshold needs, so the detector ran permanently on the absolute 300 km floor. A link judged against a gate's own history and one that merely cleared the floor are drawn differently, solid against faint, and the popup says which. The balance between them is not a constant: it moves as gates accumulate baselines, and on the worldwide feed floor-only flags fell from 76 % to 24 % of the total between 2026-08-15 and 2026-08-25. That is why the popup names the test that judged each link, rather than this page quoting a figure. The exported bundle carries the baseline **as it stood when the flag was raised**, not as it reads at export time — reading the current one produced two confident, opposite verdicts on the same link an hour apart. A gate's own bar can also climb past the 5000 km ceiling, at which point nothing can ever clear it and the gate stops flagging **silently** — 25 of 7,316 established gates were in that state when it was first measured. Those are reported, because from outside a gate that has gone deaf and a gate over a quiet band say exactly the same thing: nothing. Two or more distinct senders in the same Maidenhead field within 30 minutes become a **band-opening event** — notified via Telegram/email with an optional AI read (tropo / sporadic-E / aurora) and replayable from the map timeline. Every position is also checked against the area its own callsign prefix is allocated to — the one independent fact available about a self-reported coordinate. A link whose own position contradicts its own callsign is kept off the opening rule entirely, because the rule groups by the midpoint of the two positions: a wrong position does not weaken the evidence, it files the link in a field neither station has ever been in. Agreement is treated as weak evidence and disagreement as ambiguous between an error and an operator away from home, which is what the two directions actually support. Internet-origin, digipeated, object, balloon (>3000 m) and >5000 km (GPS garbage) packets are excluded. A link sitting on the repeated distance of a gate that measures the same value every time is kept off the opening rule as well: one gate carried 793 links at 1250.5 km ± 0.4, which is a fixed coordinate error rather than propagation, and repeated it supplies its own second sender |
