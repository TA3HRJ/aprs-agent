...
| **F-22** | group openings by **receiving gate** as well as by midpoint field. The current rule cannot see the strongest evidence there is — one gate hearing several distant unrelated senders — and the operator has already run this grouping by hand once, which is how the finding was found<br><br>**⚠️ REPORTED, NOT ACTED ON — v3.2.91.** `context.at_this_gate` asks the rule of the receiving gate beside the field grouping, so the two can be compared. Detection is unchanged, deliberately: grouping by gate means trusting gates, and the instrument for deciding whether a gate deserves it only arrived in v3.2.88–90 (F-2026-08-26-01/03/04). Now decidable, still undecided. |
