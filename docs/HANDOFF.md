...
**On a fresh machine most of them fail with `ModuleNotFoundError`**, which is
the environment and not the code: they import the app, so they need its
dependencies. `python -m venv .venv` then `.venv/Scripts/python.exe -m pip
